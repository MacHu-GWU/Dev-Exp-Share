...
简单来说, Dead Letter Queue 就是一个单独的 Queue, 专门接收那些在原来的 Queue 被多次 Consume, 但是都没有成功的 Message (默认成功之后会删除该条信息, 所以也就是说 Consumer 收到了但没有删除). 原来的 Queue 根据设定的 redirect policy, 将那些多次失败的 Message 转发到 Dead-Letter Queues.
