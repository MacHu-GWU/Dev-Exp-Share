...
def perform_hudi_upsert(
    ...
    # get the max vendorid to increment it and create update records
    max_vendor_id = spark.sql("""select max(pk_id) max_pk_id from `{}`.{} """. format(hudi_db_name, hudi_table_name)).first()[0]
    
    sql = """select * from `{}`.{} where year = 2018 and month IN (3,4) and passenger_count = 1 """.format(hudi_db_name, hudi_table_name)
    ...
    print('update rec count for the year = 2018 and month IN (6) and passenger_count = 1: ', upd_recs_df.count())
    
    
    # Generate insert data
    # get the max pk_id to increment it and create insert records
    
    
    max_pk_id = spark.sql("""select max(pk_id) max_pk_id from `{}`.{}""". format(hudi_db_name,hudi_table_name)).first()[0]
    
    print('Value for max_pk_id', max_pk_id)
    
    sql = """select * from `{}`.{} where year = 2018 and month IN (6) """.format(hudi_db_name, hudi_table_name)
    ...
    print('Insert rec count  for year = 2018 and month = 6: ', ins_recs_df.count())
    ...
    sql = """select * from `{}`.{} where year = 2018 and month IN  (12) """.format(hudi_db_name, hudi_table_name)
    ...
    print('delete rec count for year = 2018 and month IN  (12): ', delete_cnt)
    ...
    # print('Upsert Record Count {} : '.format(str(upsert_df.count())))
