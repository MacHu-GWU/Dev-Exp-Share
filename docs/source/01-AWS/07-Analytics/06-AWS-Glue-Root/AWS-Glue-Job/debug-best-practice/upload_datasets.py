...
upload /path/to/source_folder/readme.txt -> s3://{bucket}/path/to/destination_folder/readme.txt
"""
...
def upload(s3_client, src_dir_path, bucket, dst_dir_key):
    ...
    for current_dir, _, filename_list in os.walk(src_dir_path):
        for filename in filename_list:
            ...
            print("upload {} ".format(fullpath))
            print("  -> s3://{}/{}".format(bucket, s3_key))
...
if __name__ == "__main__":
    ...
    upload(
        s3_client=s3_client,
        src_dir_path=os.path.join(here, "datalake"),
        bucket="aws-data-lab-sanhe-for-everything",
        dst_dir_key="poc/2021-10-25-debug-best-practice-poc-dataset/",
    )
