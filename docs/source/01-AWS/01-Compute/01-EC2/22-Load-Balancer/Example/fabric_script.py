...
from patchwork.transfers import rsync as rsync_old
_ = rsync_old
...
HOST = "ec2-34-228-59-86.compute-1.amazonaws.com"
USER = "ec2-user"
PEM_PATH = Path.home().append_parts("ec2-pem", "eq-sanhe-dev.pem").abspath


def kill_flask_process():
    result = conn.run('ps aux | grep "flask run --host=0.0.0.0" -m 1| awk \'{print $2}\'')
    process_id = result.stdout
    conn.run('kill -9 {process_id}'.format(process_id=process_id))
...
with Connection(host=HOST, user=USER, connect_kwargs={"key_filename": [PEM_PATH, ]}) as conn:
    ""
    result = conn.run('wget -q -O - http://169.254.169.254/latest/meta-data/instance-id')
    instance_id = result.stdout
    conn.run('echo "{}" > ~/instance_id'.format(instance_id))
    conn.run('cat ~/instance_id')

    conn.run('sudo yum -y install python3')
    conn.run('which python3')
    conn.run('which pip3')
    conn.run('sudo pip3 install Flask')
    conn.run('sudo pip3 install pynamodb')
    conn.run('which flask')

    conn.put(Path(HERE, "app.py"), "/home/ec2-user/app.py")
    conn.run('export FLASK_APP=/home/ec2-user/app.py')
    conn.run('flask run --host=0.0.0.0')
    # kill_flask_process()

    # conn.run('python3 ~/app.py') # debug
