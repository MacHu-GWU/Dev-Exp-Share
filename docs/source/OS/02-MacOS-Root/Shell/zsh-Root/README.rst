...
- Mac Terminal / Iterm2: cannot display emoji icon. 有的时候默认配置在 Mac 或 Iterm2 Terminal 中不显示 emoji 小图标. 这通常是字体编码的原因. 根据 powerlevel10k 官方说明 `Recommended font: Meslo Nerd Font patched for Powerlevel10k <https://github.com/romkatv/powerlevel10k/blob/master/font.md>`_, 你下载并安装这几个字体后再在 Terminal 的 preference 把字体修改为 ``MesloLGS NF Regular`` 即可正常显示了.
